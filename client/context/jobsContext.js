...
export const JobsContextProvider = ({ children }) => {
  ...
  const updateJob = async (jobId, jobData) => {
    try {
      const res = await axios.put(`/api/v1/jobs/${jobId}`, jobData);

      toast.success("Job updated successfully");

      // Update jobs in state
      setJobs((prevJobs) =>
        prevJobs.map((job) => (job._id === jobId ? res.data : job))
      );

      // Update userJobs in state
      setUserJobs((prevUserJobs) =>
        prevUserJobs.map((job) => (job._id === jobId ? res.data : job))
      );
